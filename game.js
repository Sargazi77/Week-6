...
let randomObj = Math.floor(Math.random()*countriesAndCodes.length)
let randomObjEl =countriesAndCodes[randomObj]
randomCountryElement.innerHTML = randomObjEl.name
resultTextElement.innerHTML = ''

console.log(randomObjEl) 

// TODO display the country's name in the randomCountryElement 
randomName = randomObjEl.name
randomCode = randomObjEl["alpha-2"]
url = `https://api.worldbank.org/v2/country/${randomCode}?format=json`


// TODO add a click event handler to the submitButton.  When the user clicks the button,
//  * read the text from the userAnswerElement 
submitButton.addEventListener('click', function(){
userInput = userAnswerElement.value
fetch(url)
.then(res => res.json())
.then(data=> {
    console.log(data)
    ...
    console.log(data)
    if (userInput.toUpperCase() != Capital.toUpperCase())
    resultTextElement.innerHTML = `Wrong! The capital city of ${ randomName} is not ${userAnswerElement.value}, It is ${Capital}.`
    else 
    
    resultTextElement.innerHTML = `You're Correct the capital of ${randomName} is ${Capital}.`
